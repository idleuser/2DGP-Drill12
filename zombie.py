...
(1118, 274)]
    ...
    def have_more_ball_than_boy(self):
    ...
    def build_behavior_tree(self):
        ...
        a6 = Action('소년으로부터 멀어지기', self.move_away_from_boy)

        c1 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        c2 = Condition('공이 소년보다 많은가?', self.have_more_ball_than_boy)

        chase_boy = Sequence('소년을 추적', c1, c2, a4)
        run_away_from_boy = Sequence('소년으로부터 도망',c1, a6)
        patrol = Sequence('순찰', a5, a2)

        root = patrol = Selector('순찰 or 소년에 대한 반응', chase_boy, run_away_from_boy, patrol)
